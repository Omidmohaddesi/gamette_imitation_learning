...
class Discriminator:
    def __init__(self, config, sess):
        ...
        with tf.variable_scope('Discriminator'):
            ...
            self.expert_score, self.stu_score = self._get_score(self.expert_state, self.expert_action,
                                                                self.stu_state, self.stu_action)
            ...
            self.param = [w for w in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='Discriminator/')
                          if 'bias' not in w.name]
            print(self.param)
            with tf.get_default_graph().control_dependencies([self.train_op]):
                self.clip_op = tf.group([w.assign(tf.clip_by_value(w, - config.dis_weight_clip, config.dis_weight_clip))
                                         for w in self.param])
