...
class Policy:
    def __init__(self, config, sess, suffix):
        ...
        with tf.variable_scope('Policy_{}'.format(suffix)):
            ...
            self.action_sample = tf.clip_by_value(
                self.action_mean + self.action_std * tf.random.normal(tf.shape(self.action_mean)),
            ...
            self.ratio_clipped = tf.clip_by_value(self.ratio, 1.0 - self.config.policy_clip_range,
                                                  1.0 + self.config.policy_clip_range)
    ...
    def sample_action(self, state_2d, code, in_h_state):
        fd = {
            ...
            # self.state: np.array(list(state_2d.values())).T[:, np.newaxis, :],
            self.state: state_2d[:, np.newaxis, :],
            self.code: code
        }
    ...
    def _encode_state(self, state, in_h_state):
        ...
        for fc_dim in self.config.state_fc_dims:
            dense = tf.layers.Dense(fc_dim, activation=self.config.activation)
    ...
    def _get_action_dist_param(self, encoding):
        ...
        for dim in self.config.policy_fc_dims:
            ...
            policy_layer = dense(policy_layer)

        if self.config.scale_action:
            # action_mean = tf.layers.dense(policy_layer, self.config.action_dim, activation=tf.nn.tanh) * \
            #               self.config.action_range
            dense = tf.layers.Dense(self.config.action_dim, activation=tf.nn.sigmoid)
            action_mean = dense(policy_layer) * self.config.action_range
        else:
            # action_mean = tf.layers.dense(policy_layer, self.config.action_dim, activation=None)
            # action_mean = tf.layers.dense(policy_layer, self.config.action_dim, activation=tf.nn.sigmoid)
            dense = tf.layers.Dense(self.config.action_dim, activation=tf.nn.tanh)
            action_mean = dense(policy_layer)
        ...
        # action_log_std = tf.cast(action_log_std, dtype=tf.int32)
        # action_std = tf.cast(action_std, dtype=tf.int32)
